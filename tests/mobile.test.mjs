...
test('catalog includes fifteen unique games and all three new portrait bundles',async()=>{const games=await discoverGames(process.cwd());assert.equal(games.length,15);assert.equal(new Set(games.map(g=>g.id)).size,15);for(const kind of Object.keys(MODELS)){const g=games.find(g=>g.id===kind);assert(g);assert.equal(g.orientation,'portrait');assert(g.cover.endsWith('/cover.webp'));}});
